...
import { Sparkles, Plus, MapPin, Clock, CheckCircle2, LayoutTemplate, CalendarDays, Pencil, Trash2 } from 'lucide-react'
...
function LoadTemplateModal({
  weddingId, eventId, onClose, onDone,
}: Readonly<{ weddingId: string; eventId: string; onClose: () => void; onDone: () => void }>) {
  const { toast } = useToast()
  const [applying, setApplying] = useState<string | null>(null)

  const { data: templates = [], isLoading } = useQuery({
    queryKey: ['templates', 'APPOINTMENT'],
    queryFn: async () => {
      const res = await fetch('/api/templates?type=APPOINTMENT')
      if (!res.ok) return []
      return res.json() as Promise<{ id: string; name: string; data: unknown[] }[]>
    },
  })
    ...
    <Modal onClose={onClose} title="Load appointment template">
      <div className="space-y-3">
        ...
        {isLoading ? (
          ...
          <p className="text-sm text-zinc-400 text-center py-6">No appointment templates available</p>
        ) : (
...
function AddAppointmentModal({
  weddingId, userId, vendors, eventId, onClose, onDone,
}: Readonly<{
  ...
  const [form, setForm] = useState({
    title: '', vendorId: '', location: '', date: '',
    startTime: '09:00', endTime: '', notes: '',
  })
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      const res = await fetch(`/api/weddings/${weddingId}/appointments`, {
        ...
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          title: form.title, vendorId: form.vendorId || null,
          location: form.location || null, startAt, endAt,
          notes: form.notes || null, createdBy: userId,
...
function EditAppointmentModal({
  weddingId, appointment, vendors, onClose, onDone,
}: Readonly<{
  weddingId: string; appointment: Appointment; vendors: Vendor[]
...
function AppointmentRow({ a, weddingId, vendors, onRefresh }: Readonly<{
...
function AppointmentList({ appointments, weddingId, vendors, onRefresh }: Readonly<{
...
function OverallTab({
  appointments, events, isLoading, weddingId, vendors, onRefresh,
}: Readonly<{ appointments: Appointment[]; events: WeddingEvent[]; isLoading: boolean; weddingId: string; vendors: Vendor[]; onRefresh: () => void }>) {
  const upcoming = appointments.filter(a => a.status === 'SCHEDULED' && isFuture(new Date(a.startAt)))
  ...
  if (appointments.length === 0) return (
    <EmptyState icon={<Sparkles size={40} />} title="No appointments yet"
      description="Book appointments inside each event tab to get started" />
  )
  ...
  return (
    <div className="space-y-10">
      <div className="flex gap-8 divide-x divide-zinc-100">
        <div className="pr-8">
          <p className="text-xs font-semibold text-zinc-400 uppercase tracking-widest mb-1">Upcoming</p>
          <p className="text-2xl font-extrabold text-[#14161C]">{upcoming.length}</p>
        </div>
        {thisWeek.length > 0 && (
          <div className="px-8">
            <p className="text-xs font-semibold text-zinc-400 uppercase tracking-widest mb-1">This week</p>
            <p className="text-2xl font-extrabold text-amber-500">{thisWeek.length}</p>
          </div>
        )}
        <div className="px-8">
          <p className="text-xs font-semibold text-zinc-400 uppercase tracking-widest mb-1">Total</p>
          <p className="text-2xl font-extrabold text-zinc-400">{appointments.length}</p>
        </div>
      </div>
            ...
            <div key={key} className="rounded-2xl border border-zinc-100 p-4 flex items-center justify-between gap-4">
              ...
              <div className="flex gap-6 text-right">
                <div>
                  ...
                  <p className="text-sm font-bold text-violet-600">{upcomingCount}</p>
                </div>
                <div>
                  <p className="text-xs text-zinc-400">Total</p>
                  <p className="text-sm font-bold text-zinc-500">{appts.length}</p>
                </div>
              </div>
            </div>
          )
...
function EventAppointmentsTab({
  weddingId, userId, event, vendors, appointments, onRefresh,
}: Readonly<{
  weddingId: string; userId: string; event: WeddingEvent
  vendors: Vendor[]; appointments: Appointment[]; onRefresh: () => void
}>) {
  const [showAdd, setShowAdd] = useState(false)
  const [showTemplate, setShowTemplate] = useState(false)

  const eventAppts = useMemo(() => appointments.filter(a => a.eventId === event.id), [appointments, event.id])
  const thisWeek = eventAppts.filter(a => a.status === 'SCHEDULED' && isFuture(new Date(a.startAt)) && isThisWeek(new Date(a.startAt)))

  return (
    <div className="space-y-8">
      <div className="flex items-center justify-between flex-wrap gap-3">
        <p className="text-sm text-zinc-400">
          {eventAppts.length} appointments
      ...
      {showAdd && (
        <AddAppointmentModal
          weddingId={weddingId} userId={userId} vendors={vendors}
          eventId={event.id} onClose={() => setShowAdd(false)} onDone={onRefresh}
        />
      )}
      {showTemplate && (
        <LoadTemplateModal
          weddingId={weddingId} eventId={event.id}
          onClose={() => setShowTemplate(false)} onDone={onRefresh}
        />
      )}
    </div>
  )
...
export function AppointmentsClient({ weddingId, userId, vendors }: Readonly<Props>) {
  ...
  const refresh = () => qc.invalidateQueries({ queryKey: ['appointments', weddingId] })
  ...
  return (
    <div className="min-h-full">
      <div className="px-8 pt-10 pb-0 border-b border-zinc-100 bg-white">
        <div className="max-w-3xl mx-auto">
          ...
          <div className="flex gap-1 overflow-x-auto scrollbar-thin -mb-px">
            {isLoading ? (
              <div className="pb-4"><Spinner size="sm" /></div>
            ) : (
              tabs.map(t => (
      ...
      <div className="max-w-3xl mx-auto px-8 py-10">
        {activeTab === '__overall__' ? (
          ...
          <EventAppointmentsTab
            weddingId={weddingId} userId={userId} event={activeEvent}
            vendors={vendors} appointments={appointments} onRefresh={refresh}
          />
        ) : null}
      </div>
      ...
      {showAdd && (
        <AddAppointmentModal
          weddingId={weddingId} userId={userId} vendors={vendors}
          onClose={() => setShowAdd(false)} onDone={refresh}
        />
      )}
    </div>
  )
