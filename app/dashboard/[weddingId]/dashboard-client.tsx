  ...
  return (
    <div className="rounded-2xl border border-zinc-100 p-5 space-y-3">
      <div className="flex items-center justify-between">
        <p className="text-xs font-semibold text-zinc-400 uppercase tracking-widest">Vendors</p>
        <Link href={`/dashboard/${weddingId}/vendors`} className="text-xs font-semibold text-violet-600 hover:text-violet-800 transition-colors">View all →</Link>
      </div>

      {/* Progress */}
      <div className="flex items-end gap-2">
        <p className="text-3xl font-extrabold text-sky-600 leading-none">{confirmedVendors}</p>
        <p className="text-sm text-zinc-400 mb-0.5">of {vendorCount} confirmed</p>
      </div>
      ...
      <div className="flex gap-1 bg-zinc-100 rounded-2xl p-1">
        {STATUS_TABS.map(t => (
          <button key={t.key} onClick={() => setTab(t.key)}
            className={`flex-1 text-xs py-1.5 rounded-xl transition-all ${tab === t.key ? 'bg-white text-[#14161C] font-bold shadow-sm' : 'text-zinc-400 font-medium hover:text-zinc-600'}`}>
            {t.label}
          </button>
        ))}
      </div>
      ...
      <div className="space-y-1 max-h-40 overflow-y-auto scrollbar-thin">
        {isLoading ? (
          ...
          <p className="text-xs text-zinc-400 py-2 text-center">None in this category</p>
          ...
          return (
            <div key={v.id} className="flex items-center gap-2.5 py-1.5">
              <div className="w-6 h-6 rounded-full bg-[#CDB5F7]/20 flex items-center justify-center text-[10px] font-bold text-violet-600 flex-shrink-0">
                {v.name.charAt(0)}
              </div>
              <div className="flex-1 min-w-0">
                <p className="text-xs font-semibold text-[#14161C] truncate">{v.name}</p>
                <p className="text-[10px] text-zinc-400">{v.category.replaceAll('_', ' ')}</p>
              </div>
              {isConfirmed ? (
                ...
                <button
                  ...
                  className="flex items-center gap-1 text-[10px] font-semibold text-zinc-400 hover:text-emerald-600 hover:bg-emerald-50 rounded-full px-2 py-0.5 transition-colors flex-shrink-0 border border-zinc-200 hover:border-emerald-200"
          ...
          <div className="flex items-center gap-5">
            {wedding.couplePhotoPath ? (
              ...
              <img src={`/api/storage/signed-url?path=${encodeURIComponent(wedding.couplePhotoPath)}&bucket=media`}
                alt="Couple" className="w-20 h-20 rounded-full object-cover border-2 border-[#CDB5F7]/40 flex-shrink-0" />
            ) : (
              <div className="w-20 h-20 rounded-full bg-[#CDB5F7]/20 flex items-center justify-center text-2xl font-bold text-violet-600 flex-shrink-0">
      ...
      <div className="max-w-5xl mx-auto px-8 py-10 space-y-10">
              ...
              return (
                <div className="grid gap-0 divide-x divide-zinc-100" style={{ gridTemplateColumns: `repeat(${sliced.length}, 1fr)` }}>
                  {sliced.map(({ ev, days }, i) => {
                    ...
                    const countColor = days === 0 ? 'text-violet-600' : days <= 7 ? 'text-amber-500' : 'text-[#14161C]'
                    ...
                    return (
                      <div key={ev.id} className={`hover:opacity-70 transition-opacity ${padding}`}>
                        <div className="flex items-center gap-1.5 mb-1">
                          ...
                          <p className="text-xs font-semibold text-zinc-400 uppercase tracking-widest truncate">{ev.name}</p>
                        </div>
                        <p className={`text-4xl font-extrabold leading-none ${countColor}`}>{days === 0 ? '🎉' : days}</p>
                        <p className="text-xs text-zinc-400 mt-1.5">{countLabel}</p>
                      </div>
                    )
                  })}
                </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-[5fr_3fr] gap-10">
          ...
          <div className="space-y-4">
            ...
            {upcomingTasks.length === 0 ? (
              <p className="text-sm text-zinc-400 py-4">No upcoming deadlines in the next 30 days.</p>
            ) : (
          ...
          <div className="space-y-6">
            <Link href={`/dashboard/${wid}/appointments`}
              className="block rounded-2xl border border-zinc-100 p-5 hover:border-zinc-200 hover:bg-zinc-50/50 transition-colors space-y-3">
              <p className="text-xs font-semibold text-zinc-400 uppercase tracking-widest flex items-center gap-1.5"><CalendarCheck size={12} /> Next appointment</p>
              {nextAppointment ? (
                <>
                  ...
                  <div className="space-y-1">
                    ...
                    <p className="text-xs text-zinc-400">{formatDistanceToNow(new Date(nextAppointment.startAt), { addSuffix: true })}</p>
            ...
            <Link href={`/dashboard/${wid}/risks`}
              className={`block rounded-2xl border p-5 hover:bg-zinc-50/50 transition-colors space-y-3 ${summary.criticalRisks > 0 ? 'border-red-100 bg-red-50/40' : 'border-zinc-100'}`}>
              <div className="flex items-center justify-between">
                <p className="text-xs font-semibold text-zinc-400 uppercase tracking-widest flex items-center gap-1.5">
                  <AlertTriangle size={12} className={summary.criticalRisks > 0 ? 'text-red-400' : ''} /> Risk alerts
                </p>
                {summary.activeRisks > 0 && (
                  <span className="text-[11px] font-bold text-zinc-400">{summary.activeRisks} total</span>
                )}
              </div>
              {criticalHighRisks.length === 0 ? (
                <div>
                  <p className="text-4xl font-extrabold text-[#14161C] leading-none">{summary.activeRisks}</p>
                  <p className="text-xs text-zinc-400 mt-1">active · none critical</p>
                </div>
              ) : (
                <div className="space-y-2">
                  {criticalHighRisks.slice(0, 3).map(r => (
                    <div key={r.id} className="flex items-start gap-2">
                      ...
                      <p className="text-xs text-zinc-600 leading-relaxed line-clamp-2">{r.message}</p>
                    </div>
                  ))}
        ...
        <div className="grid grid-cols-1 lg:grid-cols-[3fr_2fr] gap-10">
          ...
          <Link href={`/dashboard/${wid}/budget`} className="space-y-4 group">
            <div className="flex items-center justify-between">
              <p className="text-xs font-semibold text-zinc-400 uppercase tracking-widest flex items-center gap-1.5"><Wallet size={12} /> Budget</p>
              <span className="text-xs font-semibold text-violet-600 group-hover:text-violet-800 transition-colors">View details →</span>
            </div>
            <div>
          ...
          <div className="space-y-8">
            ...
            <div className="space-y-3">
              <div className="flex items-center justify-between">
                <p className="text-xs font-semibold text-zinc-400 uppercase tracking-widest flex items-center gap-1.5"><Users size={12} /> RSVP</p>
            ...
            {summary.totalPledged > 0 && (
              <div className="space-y-3">
                <div className="flex items-center justify-between">
                  <p className="text-xs font-semibold text-zinc-400 uppercase tracking-widest flex items-center gap-1.5"><TrendingUp size={12} /> Contributions</p>
                  <Link href={`/dashboard/${wid}/contributions`} className="text-xs font-semibold text-violet-600 hover:text-violet-800 transition-colors">View all →</Link>
                </div>
                <div>
                  <div className="flex justify-between text-xs mb-1.5">
                    <span className="text-zinc-500">Collected of pledged</span>
                    <span className="font-bold text-[#14161C]">{contribPct}%</span>
                  </div>
                  <div className="bg-zinc-100 rounded-full h-1.5">
                    <div className="h-1.5 rounded-full bg-violet-400 transition-all duration-500" style={{ width: `${contribPct}%` }} />
                  </div>
                </div>
                <div className="flex gap-4 pt-0.5">
                  <div>
                    <p className="text-lg font-extrabold text-violet-600">{fmt(summary.totalContribPaid)}</p>
                    <p className="text-xs text-zinc-400">collected</p>
                  </div>
                  <div>
                    <p className="text-lg font-extrabold text-zinc-400">{fmt(summary.totalPledged)}</p>
                    <p className="text-xs text-zinc-400">pledged · {summary.contributorCount} members</p>
                  </div>
                </div>
              </div>
            )}
        ...
        <div className="grid grid-cols-1 lg:grid-cols-[2fr_3fr] gap-10">
          ...
          <div className="space-y-4">
            <div className="flex items-center justify-between">
              <p className="text-xs font-semibold text-zinc-400 uppercase tracking-widest flex items-center gap-1.5"><ImageIcon size={12} /> Vision board</p>
            ...
            {moodPreviews.length === 0 ? (
              <div className="rounded-2xl bg-zinc-50 border border-zinc-100 aspect-[3/4] flex items-center justify-center">
