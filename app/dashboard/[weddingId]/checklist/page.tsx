          ...
          {Array.from(byEvent.entries()).map(([key, { event, items: evItems }]) => {
            ...
            return (
              <button key={key} onClick={() => event && onSelectEvent(event.id)}
                className="w-full rounded-2xl border border-zinc-100 p-4 space-y-3 hover:border-zinc-200 hover:bg-zinc-50/50 transition-colors text-left">
                {/* Header */}
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-2 flex-wrap">
                    ...
                    <span className="text-xs text-zinc-400">{evItems.length} task{evItems.length !== 1 ? 's' : ''}</span>
                    {overdue > 0 && (
                  ...
                  <span className={`text-sm font-bold ${pct === 100 ? 'text-emerald-600' : 'text-violet-600'}`}>{pct}%</span>
                </div>
                ...
                <div className="flex items-center gap-4 flex-wrap">
                  ...
                  <span className="text-xs text-zinc-400">
                    <span className="font-semibold text-zinc-600">{pending}</span> pending
                  </span>
                  ...
                  <span className="text-xs text-violet-500 font-medium ml-auto">View tasks →</span>
                </div>
              </button>
            )
          })}
