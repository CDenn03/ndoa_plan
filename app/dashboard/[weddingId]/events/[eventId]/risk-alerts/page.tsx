...
export default function EventRiskAlertsPage(
  ...
  const resolve = async (riskId: string) => {
    try {
      await fetch(`/api/weddings/${weddingId}/risks/${riskId}`, {
        method: 'PATCH',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ isResolved: true }),
      })
      void qc.invalidateQueries({ queryKey: ['risks', weddingId] })
      toast('Risk marked as resolved', 'success')
    } catch {
      toast('Failed to resolve risk', 'error')
    }
  }
