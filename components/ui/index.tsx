...
// ─── Button ───────────────────────────────────────────────────────────────────
const btnBase = 'inline-flex items-center justify-center gap-2 rounded-xl font-semibold transition-all focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-violet-400 focus-visible:ring-offset-2 disabled:opacity-40 disabled:pointer-events-none active:scale-[0.97]'
const btnVariants = {
  primary:   'bg-[#14161C] text-white hover:bg-zinc-800',
  ...
  lavender:  'bg-[#CDB5F7]/20 text-violet-700 hover:bg-[#CDB5F7]/30',
}
...
// ─── Card ─────────────────────────────────────────────────────────────────────
// Only use where true visual separation is needed.
// Prefer spacing, Divider, or ghost variant for grouping related content.
const cardVariants = {
  default: 'bg-white rounded-2xl border border-[hsl(var(--border))]',
  ghost:   'bg-surface-subtle rounded-2xl',   // background-only, no border
...
export function CardHeader({ className, ...props }: Readonly<React.HTMLAttributes<HTMLDivElement>>) {
  return <div className={cn('px-6 py-4 border-b border-[hsl(var(--border))]', className)} {...props} />
}
...
// ─── Page Header ──────────────────────────────────────────────────────────────
// Standardises the title + subtitle + action pattern used on every page.
...
// ─── Section Label ────────────────────────────────────────────────────────────
// Replaces CardTitle for open-layout section headings.
export function SectionLabel({ children, className }: Readonly<{
  children: React.ReactNode; className?: string
}>) {
  return (
    <p className={cn('text-xs font-semibold uppercase tracking-widest text-muted-ui mb-3', className)}>
      {children}
    </p>
  )
}
...
export const Input = React.forwardRef<HTMLInputElement, React.InputHTMLAttributes<HTMLInputElement>>(
  ({ className, ...props }, ref) => (
    <input ref={ref} className={cn(
      ...
      'px-3.5 py-2 text-sm text-[#14161C] placeholder:text-zinc-400',
...
>(({ className, ...props }, ref) => (
  <textarea
    ref={ref}
    className={cn(
      'flex min-h-[80px] w-full rounded-xl border border-zinc-200 bg-white',
      'px-3.5 py-2.5 text-sm text-[#14161C] placeholder:text-zinc-400',
      'focus:outline-none focus:ring-2 focus:ring-violet-400 focus:ring-offset-1 focus:border-transparent',
...
export function Label({ className, htmlFor, children }: Readonly<{
  ...
  return (
    <label
      htmlFor={htmlFor}
      className={cn('block text-xs font-semibold text-zinc-500 uppercase tracking-wide mb-1.5', className)}
    >
      {children}
    </label>
  )
...
// ─── Stat Card ────────────────────────────────────────────────────────────────
// Open layout — no card box, just typography hierarchy.
// Use inside a flex row with divide-x for the open stat row pattern.
...
export function EmptyState({ icon, title, description, action }: Readonly<{
  ...
  return (
    <div className="flex flex-col items-center justify-center py-20 text-center">
      ...
      <h3 className="text-base font-semibold text-zinc-500">{title}</h3>
      {description && (
        <p className="text-sm text-zinc-400 mt-1.5 max-w-xs leading-relaxed">{description}</p>
      )}
...
export function Spinner({ size = 'md' }: Readonly<{ size?: 'sm' | 'md' | 'lg' }>) {
  ...
  return (
    <div className={cn('animate-spin rounded-full border-2 border-zinc-100 border-t-violet-500', s[size])} />
  )
}
...
export function Modal({ onClose, title, children }: Readonly<{
  ...
  return (
    <div className="fixed inset-0 bg-black/30 backdrop-blur-sm z-50 flex items-end sm:items-center justify-center p-4">
      <div className="bg-white rounded-2xl w-full max-w-md shadow-sm sm:animate-none modal-slide-up">
        <div className="flex items-center justify-between px-6 py-5 border-b border-[hsl(var(--border))]">
          ...
          <button
            onClick={onClose}
            className="w-8 h-8 rounded-lg flex items-center justify-center hover:bg-zinc-100 text-zinc-400 transition-colors"
          >
